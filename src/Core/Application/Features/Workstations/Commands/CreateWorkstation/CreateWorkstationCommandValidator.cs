...
namespace Application.Features.Workstations.Commands.CreateWorkstation
{
    public class CreateWorkstationCommandValidator : AbstractValidator<CreateWorkstationCommand>
    {
        ...
        public CreateWorkstationCommandValidator(IRepositoryWrapper repository, IMapper mapper)
        {
        ...
        private async Task<bool> IsUnique(CreateWorkstationCommand workstationCommand, CancellationToken cancellationToken)
        {
            var workstation = _mapper.Map<Workstation>(workstationCommand);
            return await _repository.Workstation.ExistAsync(workstation);
        }
    }
}
