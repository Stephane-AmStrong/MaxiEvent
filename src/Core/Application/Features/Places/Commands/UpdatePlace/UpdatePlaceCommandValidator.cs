...
namespace Application.Features.Places.Commands.UpdatePlace
{
    public class UpdatePlaceCommandValidator : AbstractValidator<UpdatePlaceCommand>
    {
        ...
        public UpdatePlaceCommandValidator(IRepositoryWrapper repository, IMapper mapper)
        {
        ...
        private async Task<bool> IsUnique(UpdatePlaceCommand placeCommand, CancellationToken cancellationToken)
        {
            var place = _mapper.Map<Place>(placeCommand);
            return await _repository.Place.PlaceExistAsync(place);
        }
    }
}
