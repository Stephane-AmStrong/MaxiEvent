...
namespace Application.Features.Places.Commands.CreatePlace
{
    public class CreatePlaceCommandValidator : AbstractValidator<CreatePlaceCommand>
    {
        ...
        public CreatePlaceCommandValidator(IRepositoryWrapper repository, IMapper mapper)
        {
        ...
        private async Task<bool> IsUnique(CreatePlaceCommand placeCommand, CancellationToken cancellationToken)
        {
            var place = _mapper.Map<Place>(placeCommand);
            return await _repository.Place.ExistAsync(place);
        }
    }
}
